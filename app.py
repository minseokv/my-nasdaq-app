...
<style>
    ...
    .combined-score-container {
        display: flex; background-color: #161618; border: 2px solid #333;
        border-radius: 15px; overflow: hidden; min-height: 160px; margin-bottom: 25px;
    }
    .score-part {
        flex: 1; display: flex; flex-direction: column; align-items: center;
        justify-content: center; border-right: 1px solid #333; background-color: #1E1E1E;
    }
    .guide-part { flex: 2; padding: 25px; display: flex; flex-direction: column; justify-content: center; text-align: left; }
...
# ---------------------------------------------------------
# 3. 위젯 모드 판별 및 출력 (이 부분이 채널 전환 역할)
# ---------------------------------------------------------
# 점수별 멘트 설정
if curr_score <= 20: g_t, g_d, g_c = "🚨 인생 역전", "TQQQ 50% 이상 매수!!!", "#4CAF50"
elif curr_score <= 30: g_t, g_d, g_c = "🛒 강력 매수", "TQQQ 30% 매수!!", "#81C784"
elif curr_score <= 40: g_t, g_d, g_c = "🌱 가벼운 매수", "TQQQ 10% 매수!", "#A5D6A7"
elif curr_score >= 85: g_t, g_d, g_c = "📉 수익 실현", "20% 매도해 현금 확보", "#EF5350"
else: g_t, g_d, g_c = "💤 적립 유지", "평범한 중립 구간입니다.", "#00FFD1"

# 주소 뒤에 ?view=widget 이 붙어있을 때만 실행되는 코드
if st.query_params.get("view") == "widget":
    ...
    """, unsafe_allow_html=True)
    st.stop() # 위젯 모드일 때는 여기서 끝! (아래 사이트 내용은 안 나옴)

# ---------------------------------------------------------
# 4. 전체 사이트 UI 구성 (일반 접속 시 보임)
# ---------------------------------------------------------
...
with c_top_R:
    ...
    cw1.metric("심리(RSI)", f"{rsi:.1f}"); cw2.metric("자금(MFI)", f"{mfi:.1f}"); cw3.metric("밴드위치", f"{pb:.1f}%")
        ...
        for day in week:
            ...
            try:
                ...
                if not row.empty:
                    ...
                    if d_str == datetime.now().strftime('%Y-%m-%d'): brd = "2px solid #FFFFFF"
            except: pass
...
<div style="background-color: #161618; padding: 15px; border-radius: 10px; border: 1px solid #333; margin-top: 10px;">
    <p style="margin: 0; font-size: 14px; color: #E0E0E0; font-weight: bold;">🇺🇸 미국 증시 영업시간 (한국 기준: 월요일 밤 ~ 토요일 새벽)</p>
    <div style="display: flex; gap: 20px; margin-top: 8px;">
        <div style="flex: 1; font-size: 13px; color: #AAA;"><span style="color: #00FFD1;">●</span> <b>서머타임 (3월~11월)</b><br>- 정규장: 22:30 ~ 05:00</div>
        <div style="flex: 1; font-size: 13px; color: #AAA;"><span style="color: #EF5350;">●</span> <b>평시 (11월~3월)</b><br>- 정규장: 23:30 ~ 06:00</div>
    </div>
</div>
""", unsafe_allow_html=True)
