...
with col_b1:
    ...
    # [수정] 폰에서도 한 줄에 나오도록 컬럼 비율 최적화
    nb1, nb2, nb3 = st.columns([1, 2, 1])
    with nb1: st.button("◀", key="prev", on_click=move_calendar, args=(-1,))
    with nb2: st.markdown(f"<div style='text-align:center; font-weight:bold; padding-top:10px;'>{st.session_state.cal_year}년 {st.session_state.cal_month}월</div>", unsafe_allow_html=True)
    with nb3: st.button("▶", key="next", on_click=move_calendar, args=(1,))
...
with col_b2:
    ...
    st.markdown(f'<div style="font-size:13px; color:#AAA; text-align:right; margin-bottom:5px;">💰 40↓: <b style="color:#4CAF50;">{b_d}회</b> | 🔥 85↑: <b style="color:#EF5350;">{s_d}회</b></div>', unsafe_allow_html=True)
...
<div style="background-color: #161618; padding: 15px; border-radius: 10px; border: 1px solid #333; margin-top: 15px;">
    <p style="margin: 0; font-size: 13px; color: #E0E0E0; font-weight: bold;">🇺🇸 미국 증시 영업시간 (한국기준)</p>
