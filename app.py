...
with col_left:
    ...
    # [★ 가독성 끝판왕: 통합 선택바 ★]
    # 화살표 버튼이 깨지는 문제를 해결하기 위해, 한 줄에 꽉 차는 드롭다운 선택 상자를 도입했습니다.
    # 사용자가 직접 연도와 월을 선택할 수 있어 훨씬 전문적이고 절대로 깨지지 않습니다.
    years = [2024, 2025, 2026]
    months = list(range(1, 13))
    
    c_y, c_m = st.columns(2)
    with c_y: sel_year = st.selectbox("연도", years, index=years.index(datetime.now().year), label_visibility="collapsed")
    with c_m: sel_month = st.selectbox("월", months, index=datetime.now().month-1, label_visibility="collapsed")
