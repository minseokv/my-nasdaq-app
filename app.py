...
# 2. 갤럭시 S24 정밀 타격: 소형 버튼 및 가로 고정 CSS
st.markdown("""
<style>
    /* 전체 배경 */
    ...
    .basis-item { padding: 8px 0; text-align: center; border-right: 1px solid #222; }
    .basis-item:last-child { border-right: none; }
    .basis-label { font-size: 11px; color: #888; margin-bottom: 2px; }
    .basis-value { font-size: 15px; font-weight: bold; color: #00FFD1; }

    /* [3] ★ 달력 내비게이션: 소형 버튼 + 가로 고정 ★ */
    /* 폰에서 컬럼이 꺾이는 것을 막기 위해 강제 명령 */
    ...
    /* 민석님이 요청한 '픽셀이 작은 버튼' 설정 */
    .stButton > button {
        background-color: #1E1E1E !important;
        color: #00FFD1 !important;
        border: 1px solid #00FFD1 !important;
        border-radius: 4px !important;
        width: 35px !important;  /* 버튼 너비를 아주 작게 고정 */
        height: 32px !important; /* 버튼 높이도 슬림하게 */
...
with col_l:
    ...
    cn1, cn2, cn3 = st.columns([1, 5, 1])
    with cn1: st.button("◀", key="prev_c", on_click=move_cal, args=(-1,))
    with cn2: st.markdown(f"<div style='text-align:center; font-weight:bold; font-size:17px; line-height:32px;'>{st.session_state.cal_year}년 {st.session_state.cal_month}월</div>", unsafe_allow_html=True)
    with cn3: st.button("▶", key="next_c", on_click=move_cal, args=(1,))
