...
if not df.empty:
    ...
    # [A] AI 점수 통합 박스 (3단계 가이드 반영)
    if curr_score <= 20:
        g_title, g_desc, g_color = "🚨 인생 역전 기회", "시장이 공포에 질렸습니다.<br><span class='point-red'>TQQQ 50% 매수</span>!!!", "#4CAF50"
    elif curr_score <= 30:
        g_title, g_desc, g_color = "🛒 강력 매수", "확실한 저가 매수 찬스입니다.<br><span class='point-red'>TQQQ 30% 매수</span>!!", "#81C784"
    elif curr_score <= 40:
        g_title, g_desc, g_color = "🌱 가벼운 매수", "건강한 조정 구간입니다.<br><span class='point-red'>TQQQ 10% 매수</span>!", "#A5D6A7"
    elif curr_score >= 85:
        g_title, g_desc, g_color = "📉 수익 실현 권장", "상승의 끝자락일 수 있습니다.<br><span class='point-red'>20% 매도</span>해 현금을 확보하세요.", "#EF5350"
    else:
        g_title, g_desc, g_color = "💤 적립 유지", "평범한 우상향 구간입니다.<br>매일 QLD 만원 적립을 유지하세요.", "#00FFD1"
    ...
    with c_top_L:
        ...
        <div class="combined-score-container">
            ...
            <div class="guide-part">
                <h3 style="margin:0; color:{g_color} !important;">{g_title}</h3>
                <p style="margin-top:8px; color:#BBB; font-size:15px; line-height:1.5;">{g_desc}</p>
            </div>
        </div>
        """, unsafe_allow_html=True)
    with c_top_R:
        ...
        cw1.metric("심리(RSI)", f"{rsi:.1f}")
        cw2.metric("자금(MFI)", f"{mfi:.1f}")
        cw3.metric("밴드위치", f"{pb:.1f}%")
        st.info("RSI(30%), MFI(30%), 밴드위치(40%)를 종합 분석한 통계값입니다.")
    ...
    with col_cal:
        ...
        with nc1: 
            if st.button("◀", key="p"): move_calendar(-1)
        with nc3: 
            if st.button("▶", key="n"): move_calendar(1)
    ...
    with e1: st.markdown(f'<div style="{c_css}"><h4>🧠 심리 (RSI)</h4><p style="font-size:15px; color:#AAA; margin-top:8px;">시장의 과열 정도를 측정합니다.</p></div>', unsafe_allow_html=True)
    with e2: st.markdown(f'<div style="{c_css}"><h4>💰 자금 (MFI)</h4><p style="font-size:15px; color:#AAA; margin-top:8px;">거래량을 포함한 자금 유입 지표입니다.</p></div>', unsafe_allow_html=True)
    with e3: st.markdown(f'<div style="{c_css}"><h4>📏 밴드위치 (%B)</h4><p style="font-size:15px; color:#AAA; margin-top:8px;">평균 대비 현재 주가의 위치입니다.</p></div>', unsafe_allow_html=True)
    ...
    <div style="background-color: #161618; padding: 15px; border-radius: 10px; border: 1px solid #333; margin-top: 10px;">
        ...
        <div style="display: flex; gap: 20px; margin-top: 8px;">
            <div style="flex: 1; font-size: 13px; color: #AAA;">
                <span style="color: #00FFD1;">●</span> <b>서머타임 (3월~11월)</b><br>- 정규장: 22:30 ~ 05:00<br>- 프리마켓: 17:00 ~ 22:30
            </div>
            <div style="flex: 1; font-size: 13px; color: #AAA;">
                <span style="color: #EF5350;">●</span> <b>평시 (11월~3월)</b><br>- 정규장: 23:30 ~ 06:00<br>- 프리마켓: 18:00 ~ 23:30
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
