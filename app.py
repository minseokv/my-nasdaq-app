...
<style>
    ...
    .ms-title-area { margin-bottom: 25px; text-align: left; }
    .ms-title-top { font-size: 26px; font-weight: 800; display: flex; align-items: center; gap: 10px; }
    .ms-title-bottom { padding-left: 38px; font-size: 26px; font-weight: 800; margin-top: -5px; display: block; }
    ...
    .basis-node { padding: 10px 0; text-align: center; border-right: 1px solid #222; }
    .basis-node:last-child { border-right: none; }
    .basis-lab { font-size: 11px; color: #888; margin-bottom: 4px; }
    .basis-val { font-size: 16px; font-weight: bold; color: #00FFD1; }

    /* [3] ★ 캘린더 네비게이션: 표 중앙 상단 배치 ★ */
    ...
    /* [4] ★ 흐름 분석: 테두리 있는 버튼 스타일 복구 ★ */
    .period-box div[data-testid="stRadio"] > div[role="radiogroup"] {
        display: flex !important;
        flex-direction: row !important;
        flex-wrap: wrap !important;
        gap: 8px !important;
    }
    .period-box div[data-testid="stRadio"] > div[role="radiogroup"] > label {
        flex: none !important;
        background-color: #1E1E1E !important;
        border: 1px solid #444 !important; /* 회색 테두리 추가 */
    ...
    /* 달력 본체 폭 고정 */
    .cal-grid-area { max-width: 380px; margin: 0 auto; }
    ...
    @media (max-width: 768px) {
        ...
        .cal-grid-area { max-width: 100% !important; }
    }
...
# [근거 그리드]
st.markdown(f"""<div class="basis-box"><div class="basis-node"><div class="basis-lab">심리(RSI)</div><div class="basis-val">{last_row['RSI']:.1f}</div></div><div class="basis-node"><div class="basis-lab">자금(MFI)</div><div class="basis-val">{last_row['MFI']:.1f}</div></div><div class="basis-node"><div class="basis-lab">밴드위치</div><div class="basis-val">{last_row['PctB']:.1f}%</div></div></div>""", unsafe_allow_html=True)
...
with col_l:
    ...
    st.markdown('<div class="cal-grid-area">', unsafe_allow_html=True)
