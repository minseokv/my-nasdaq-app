...
# 2. 고강도 정밀 CSS (캘린더 버튼 한 줄 고정 및 제목 정렬)
st.markdown("""
<style>
    ...
    /* [1] 제목: 호랑이 옆 '민석이의' 아래에 '나스닥100 투자' 정밀 정렬 */
    .main-title {
        font-size: 26px;
        font-weight: 800;
        line-height: 1.4;
    ...
    .indented-line {
        padding-left: 38px; /* 호랑이 아이콘 너비만큼 정밀 들여쓰기 */
        display: block;
    }
    ...
    /* [3] ★ 캘린더 버튼 한 줄 고정 (초강력 해결책) ★ */
    /* 폰에서 컬럼이 세로로 꺾이는 스트림릿 기본 기능을 강제로 끕니다 */
    .cal-nav-row [data-testid="stHorizontalBlock"] {
    ...
    .cal-nav-row [data-testid="column"] {
...
<div class="combined-score-container">
    <div class="score-part"><span style="color:#888; font-size:13px;">현재 AI 점수</span><span style="color:#00FFD1; font-size:70px; font-weight:bold; line-height:1;">{curr_score}</span></div>
    <div class="guide-part"><h3 style="margin:0; color:{g_c} !important;">{g_t}</h3><p style="margin-top:8px; color:#BBB; font-size:15px; line-height:1.5;">{g_d}</p></div>
</div>
""", unsafe_allow_html=True)
...
with col_mid1:
    ...
    # [수정] ★ nav-row-container로 감싸서 폰에서도 무조건 한 줄 유지 ★
    ...
    with c_nav2: st.markdown(f"<div style='text-align:center; font-weight:bold; font-size:18px; padding-top:10px;'>{st.session_state.cal_year}년 {st.session_state.cal_month}월</div>", unsafe_allow_html=True)
...
st.markdown("""<table class="strategy-table"><thead><tr><th>구간</th><th>상태</th><th>Action Plan</th></tr></thead><tbody><tr><td style="color:#4CAF50;">0~20</td><td>🚨공황</td><td>인생역전: <span class="point-red">50% TQQQ 매수</span></td></tr><tr><td style="color:#81C784;">21~30</td><td>🛒침체</td><td>저가매수: <span class="point-red">30% TQQQ 매수</span></td></tr><tr><td style="color:#A5D6A7;">31~40</td><td>🌱조정</td><td>가벼운매수: <span class="point-red">10% TQQQ 매수</span></td></tr><tr><td style="color:#00FFD1;">41~84</td><td>💤중립</td><td><b>QLD 적립 유지</b></td></tr><tr><td style="color:#EF5350;">85~100</td><td>🔥과열</td><td>수익실현: <span class="point-red">20% 매도</span></td></tr></tbody></table>""", unsafe_allow_html=True)
