...
<style>
    ...
    .basis-item { padding: 10px 5px; border-right: 1px solid #333; }
    ...
    /* [3] 달력 & 버튼: 폰에서도 무조건 한 줄 정렬 */
    [data-testid="column"] { min-width: 0px !important; } /* 컬럼 최소폭 해제 */
    [data-testid="stHorizontalBlock"] { flex-wrap: nowrap !important; align-items: center !important; } /* 가로 정렬 강제 */
    ...
    /* [4] PC 캘린더 크기 제한 (너무 커지지 않게) */
    .cal-container {
        max-width: 450px; /* PC에서 달력 폭 고정 */
        margin: 0 auto;
    }

    /* [5] 흐름 분석 라디오 버튼 그리드 (3x2) */
    ...
    .score-part { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; border-right: 1px solid #333; background-color: #1E1E1E; padding: 20px; }
...
# [상단] AI 점수 통합 박스
if curr_score <= 20: g_t, g_d, g_c = "🚨 인생 역전 기회", "TQQQ 50% 매수!!!", "#4CAF50"
elif curr_score <= 30: g_t, g_d, g_c = "🛒 강력 매수", "TQQQ 30% 매수!!", "#81C784"
elif curr_score <= 40: g_t, g_d, g_c = "🌱 가벼운 매수", "TQQQ 10% 매수!", "#A5D6A7"
elif curr_score >= 85: g_t, g_d, g_c = "📉 수익 실현 권장", "20% 매도해 현금 확보", "#EF5350"
else: g_t, g_d, g_c = "💤 적립 유지", "평범한 중립 구간입니다.", "#00FFD1"
...
c_nav_col1, c_nav_col2, c_nav_col3 = st.columns([1, 3, 1])
with c_nav_col1: st.button("◀", key="prev_cal", on_click=move_calendar, args=(-1,))
with c_nav_col2: st.markdown(f"<div style='text-align:center; font-weight:bold; font-size:18px; padding-top:8px;'>{st.session_state.cal_year}년 {st.session_state.cal_month}월</div>", unsafe_allow_html=True)
...
st.markdown(f'<div style="font-size:13px; color:#AAA; text-align:right; margin-bottom:10px;">💰 40↓: <b style="color:#4CAF50;">{b_d}회</b> | 🔥 85↑: <b style="color:#EF5350;">{s_d}회</b></div>', unsafe_allow_html=True)
